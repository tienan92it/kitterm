...
| **Browse for context** | ⌘⌥O opens a small browser at your cursor, on the machine the session runs on. Arrows move, → opens a folder, ← goes up, ⏎ inserts the real path — no copy, so an agent edits the actual file. ⇧⏎ takes the folder itself |
