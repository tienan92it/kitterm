...
| 4 | **Delete the escape hatch.** Remove `KNOWN_BELOW` and its plumbing, so a failing pair is a failing test with no way to record it as expected. | The mechanism is gone; a planted bad pair fails the suite. |
