...
| # | Capability | Proof |
|---|---|---|
| 1 | **Project identity on the daemon.** `~/.kitterm/projects.json` (`{version, projects:[{id, name, root, knowledge?}]}`), longest-prefix then `.git` walk with a `gitdir:` follow, resolved when `liveCwd` changes. Row fields `project {id, name, root, registered}` and `orchestrated`. Reserved label keys `project`, `goal`, `round`. `GET /api/projects` with counts by `mergedState`, pending approvals, `lastOutputAt`, archive count. `?project=` on sessions and archives. `kitterm project add / list / remove`. | Resolution table test: registered wins, git walk, worktree `gitdir:`, no project, label override. Aggregate route test. Bench unchanged. |
| 2 | **The dashboard.** `sessions-model.ts` with pure `group`, `sortInGroup`, `filter`, `attention`. Attention strip. One card per project with counts and spawn-in-project. Rows with kill and archive behind a confirm. Crew sub-headers. Archived rows under their project. Filter chips and text search. Phone layout at 390 px. Link from the settings panel. Poll becomes `/api/projects` plus `/api/sessions`. | Vitest on every pure function. Corpus request `01-phone-dashboard`. |
...
| 4 | **The goal-loop skill and skill install.** `examples/foreman/goal-loop.md` holds the round procedure from `LOOP.md` as a prompt. `foreman-loop`, `review-crew`, `triage` post into the round record when a `goal:` label is set. `kitterm skills install` copies `examples/foreman/*` into `~/.claude/skills/` and reports the diff. | CLI test: install, second run reports no change. Completion condition 3. |
...
| 6 | **Dogfood.** Run capability 2 as a goal through `goal-loop` with a three-round budget. Fold the reflections into `facts.md` and the skills. | Corpus request `03-goal-loop-dogfood`. Completion condition 4. |
