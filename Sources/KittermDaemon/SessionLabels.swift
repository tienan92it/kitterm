...
public struct SessionLabels: Sendable, Equatable {
    ...
    /// The keys the goal loop reserves; the Labels table in
    /// `docs/goals/LOOP.md` is the one list of them and what each holds.
    /// `project:<id>` overrides the project the daemon resolves from a
    /// session's cwd (`ProjectStore`); the daemon passes the others through.
