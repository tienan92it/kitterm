...
- **Foreman** — `kitterm mcp` is a stdio MCP server that hands an ordinary agent the crew-management toolset (spawn/name/watch/message/wait/note over the HTTP API); the daemon holds no orchestration logic. `docs/foreman.md` is the operating manual and `examples/foreman/` holds reference skills. The boundary is deliberate: kitterm ships the machinery, the foreman is a prompt
