...
| 6 | **Sink the well.** `--ui-bg-sunken` is the file preview's background. `rounds/005.md` measures its group at 19 entries, of which 13 clear when the well darkens and 6 fail there too. Darken it so those 13 clear. The preview paints the theme's own ANSI colours as syntax, and `goal.md` does not let this goal change them, so the well is the only lever. Keep the preview readable as a sunken panel: it exists to look recessed against `--ui-bg`, and a well that matches the page loses that. | 13 entries gone from `KNOWN_BELOW` and 0 new; the remaining 6 record that only a theme's own colours could move them; the well still reads as recessed against `--ui-bg`, measured on the two themes that move most; a before-and-after of the file preview. |
