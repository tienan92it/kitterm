...
| 3 | **Nearest `.git` wins.** `ProjectStore.resolve` takes the nearest ancestor of the cwd that is either a checkout or a registered root, so a checkout under a registered parent is its own project and a registered root inside a checkout still names and configures its own tree. Outside every checkout the longest registered root still wins. | The resolution table gains the nested case and the registered-root-inside-a-checkout case; the existing rows still pass; `kitterm project list` unchanged. |
