...
| 5 | **Every line is one line, and its state is unmistakable.** Every state renders as a bracketed word beside its coloured mark, and the `SESSIONS` header prints the vocabulary. The mark of a line whose agent holds the tty turns through the ten braille frames at 80 ms, stopped by `prefers-reduced-motion`. The tree's line component carries mark, indent, name, facts, time and actions, and its facts drop from the right in a fixed order as the line narrows. Nothing wraps at 390 px. | A vitest over the line model for each level and each drop step, and over the state vocabulary: every state yields its bracketed word and its mark. A `sessions-css.test.ts` assertion that the only `animation` in the file is the spinner's, and that it sits inside a `prefers-reduced-motion` guard. A vitest over the line model for each level and each drop step. A measurement in the live check: every line's `scrollHeight` equals one `--line-h` at 390 px. The page at 390 px is under 1200 px tall with the live tree loaded. |
...
| 7 | **The page says what the spend bought.** A `yield` block shows the range's merged pull requests, merged lines, releases and hours of model time, each with its unit cost, and says in the same line that a line and a PR are proxies. The panel groups by project, goal, task or role, the same levels the tree has: a project's spend from the rollup, a goal's from the sum of its records' `Cost:` lines, a task's from its own round, a role's from the transcript's directory. A row with no source prints a dash, never a zero, and the unattributed remainder is a row in amber, not a footnote. A `leaks` block names the spend with no round record, the corrections per round, and any session under 95% cached. The cache share leaves every heading and becomes that exception line. | Swift tests over the role reader (a worktree path, a root path, a path outside every project) and over the yield arithmetic with a zero-count range. A vitest over the grouping model at each of the four levels, including a group with no spend, a group with no pull request, the unattributed row, and an empty range. A `sessions-css.test.ts` assertion that no heading carries a cache share. A screenshot against the real tree. |
