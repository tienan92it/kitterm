...
| **Browse for context** | ⌘⌥O opens a file browser on the machine the session runs on. Pick a file or a folder and its real path lands at your cursor — no copy, so an agent edits the actual file |
